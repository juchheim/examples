...
foreach($assignments as $assignment) {
    ...
    if ($result = $mysqli -> query("SELECT * FROM ass_viewed WHERE id = '".$id."'")) {
        ...
        if($result -> num_rows == 0 and $assignment->get_viewed() == 0) {
            $sql = "INSERT INTO ass_viewed (id, id_user, id_file)
            VALUES ('".$id."', '".$id_user."', '".$id_file."')";
...
echo "finished updating unviewed assignments<br><br>";
...
$delete_table = $mysqli -> query("DELETE FROM ass_printout_requested");
$mysqli->query($delete_table);
...
foreach ($user_logins as $key => $value) {
    ...
    while($podFile->fetchRecord()) {
        ...
        while($podUsers->fetchRecord()) {
            ...
            # get all of this user's viewed documents
            $viewed_documents = $podUsers->get_field('printout_requested');
            # put viewed into array
            foreach($viewed_documents as $view) {
                array_push($views, $view[post_title]);
            }
            ...
            {
                # add assignment object to array
                array_push($assignments_p, $assignment);
                $completed_count++;
            } else {
                $skipped_count++;
...
$i = 0;
foreach($assignments_p as $assignment) {
    # get id's
    $id_user = $assignments_p[$i]->id_user;
    $id_file = $assignments_p[$i]->id_file;

    # combine id's into assignment id
    $id = "user". $id_user ."file". $id_file;

    # get other object data
    $has_viewed = $assignments_p[$i]->viewed;

    # get object data to insert into users table
    $user_login = $assignments_p[$i]->user_login;
    $user_first_name = addslashes($assignments_p[$i]->user_first_name);
    $user_last_name = addslashes($assignments_p[$i]->user_last_name);
    ...
    # get object data to insert into media table
    $file_name = $assignments_p[$i]->file_name;
    ...
    # get all assignments with current id
    if ($result = $mysqli -> query("SELECT * FROM ass_printout_requested WHERE id = '".$id."'")) {
        # if assignment isn't in assignments and isn't viewed
        if($result -> num_rows == 0) {
            $sql = "INSERT INTO ass_printout_requested (id, id_user, id_file)
            VALUES ('".$id."', '".$id_user."', '".$id_file."')";

            if ($mysqli->query($sql) === TRUE) {
                echo $id .": new printout request added.<br>";
            } else {
                echo "Error: " . $sql . "<br>" . $conn->error;
            }
        }
